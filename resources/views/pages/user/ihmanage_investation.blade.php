<!-- D:\web_dana_syariah.git\resources\views\pages\user\ihmanage_investation.blade.php -->
...
<div class="modal fade" id="addPackageModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-dialog-centered" role="document">
	  <div class="modal-content">
		<div class="modal-body">
		  <form action="{{route('addActive')}}" method="POST" id="formTambahPaket">
			@csrf
		...
		<div class="modal-footer">
		  <button type="button" class="btn btn-sm btn-secondary" data-dismiss="modal">Batal</button>
		  <button type="button" class="btn btn-sm btn-primary" id="tambahPaket">Tambah Paket</button>
		</div>
	  </form>
	  </div>
	</div>
...
<div class="modal fade" id="ambilDanaModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-dialog-centered" role="document">
	<div class="modal-content">
	  <div class="modal-body">
		<form action="{{route('ihambilActive')}}" method="POST" id="formAmbilDana">
		  @csrf
	  ...
	  <div class="modal-footer">
		<button type="button" class="btn btn-sm btn-secondary" data-dismiss="modal">Batal</button>
		<button type="button" class="btn btn-sm btn-primary" id="ambilDana">Ambil Dana</button>
	  </div>
	</form>
	</div>
  </div>
...
$(document).ready(function(){
        ...
        $.ajax({
            ...
            success:function(data)
            {
              console.log(data)
                ...
                var allit = formatNumber(parseInt(detil.prospek_hasil_diterima));
                // #backup
                ...
                $.each(detil_get,function(index,value){
                    ...
                    if(xx == lastkey){
                      if(marginprofit != 0.00){
                        ...
                        // #backup }else if (value.status_payout == 4){
                          ...
                          $("#urutan").append("<td colspan='2'> <h6><span class='badge badge-info'> Sisa Imbal Hasil</span><h6> </td>");
                          // #backup $("#urutan").append("<td colspan='2'> <h6><span class='badge badge-info'> Sisa Imbal Hasil Akhir </span><h6> </td>");
                        }
                        $("#urutan").append("</tr>");
                      }
                    }else if(xx == lastkey1){
                      ...
                      $("#urutan").append("<td>Rp. " + formatNumber(parseInt(value.imbal_payout)) + "</td>");
                      // #backup if (value.status_payout == 4){
                      if (value.status_payout == 3){
                      ...
                      $("#urutan").append("<tr>");
                      if(xx == 1){
                        $("#urutan").append("<td> Bulan 1 + Proposional</td>");
                      }else{
                        $("#urutan").append("<td> Bulan " + xx + "</td>");
                      }
                      //tanggal imbal hasil
                      // #backup if(value.status_update >= 1){
                      //   #backup $("#urutan").append("<td>" + value.tgl_update + " <i>" + value.ket_weekend + "</i></td>");
                      if(value.ket_libur === " "){
                        $("#urutan").append("<td>" + value.tanggal_payout +"</td>");
                      }else{
                        $("#urutan").append("<td>" + value.tanggal_payout + " <i> hari sebelumnya " + value.ket_libur + "</i></td>");
                        // #backup $("#urutan").append("<td style="+(value.status_libur == 1 ? 'background-color:red' : '')+";"+(value.status_libur == 1 ? 'color:white !important;' : '')+">" + value.tanggal_payout + " <i>" + value.ket_weekend + "</i></td>");
                      }
                      // nominal imbal hasil
                      $("#urutan").append("<td>Rp. " + formatNumber(parseInt(value.imbal_payout)) + "</td>");
                      // status imbal hasil
    ...
    $('#tambahPaket').one('click',function(){
        $('#formTambahPaket').submit();
    })

    $('#ambilDana').one('click',function(){
        $('#formAmbilDana').submit();
    })
